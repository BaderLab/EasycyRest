...
#' attributes
#' @param id_column (char) column of table to set for id,name and shared_name
#' @examples
...
#' @export
nodeSet2JSON <- function(node_set, id_column="Alias"){
  json_nodes <- c()
  for(i in 1:dim(node_set)[1]){
    ...
    rest <- c(node_set[i,id_column],
	node_set[i,id_column],
	node_set[i,id_column],
	rest)
